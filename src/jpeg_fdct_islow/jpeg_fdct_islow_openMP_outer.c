...
int main(void){
    ...
    int *elem = (int *)malloc(TOTAL_FUNC_CALL*8*8*(sizeof(int)));
    int *dataptr = (int *)malloc(TOTAL_FUNC_CALL*8*8*(sizeof(int)));
    ...
    for(int i = 0 ; i<TOTAL_FUNC_CALL ; i++){
        int offset = i*DCTSIZE*DCTSIZE;
        func_islow(&elem[offset], &dataptr[offset]);
    }
    gettimeofday(&end, NULL);
    printf("done processing data\n");

    double time_jpeg_fdct_islow = ((double) end.tv_sec - (double) start.tv_sec) + ((double) end.tv_usec - (double) start.tv_usec)/1000000.0;
