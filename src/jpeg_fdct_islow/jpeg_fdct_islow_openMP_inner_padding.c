...
void func_islow(int *elemptr, DCTELEM * dataptr){
    ...
    int elemptrp[DCTSIZE2][PAD_SIZE];
    int dataptrp[DCTSIZE2][PAD_SIZE];

    // #pragma omp parallel for
    for(int i  = 0 ; i<DCTSIZE2 ; i++){
        elemptrp[i][0] = elemptr[i];
    }
    ...
    for(ctr = 0 ; ctr<DCTSIZE ; ctr++){
        ...
        tmp0 = GETJSAMPLE(elemptrp[offset+0][0]) + GETJSAMPLE(elemptrp[offset+7][0]);
        tmp1 = GETJSAMPLE(elemptrp[offset+1][0]) + GETJSAMPLE(elemptrp[offset+6][0]);
        tmp2 = GETJSAMPLE(elemptrp[offset+2][0]) + GETJSAMPLE(elemptrp[offset+5][0]);
        tmp3 = GETJSAMPLE(elemptrp[offset+3][0]) + GETJSAMPLE(elemptrp[offset+4][0]);
        ...
        dataptrp[offset+0][0] = (DCTELEM) ((tmp10 + tmp11 - 8 * CENTERJSAMPLE) << PASS1_BITS);
        dataptrp[offset+4][0] = (DCTELEM) ((tmp10 - tmp11) << PASS1_BITS);

        z1 = MULTIPLY(tmp12 + tmp13, FIX_0_541196100);       /* c6 */
        
        z1 += ONE << (CONST_BITS-PASS1_BITS-1);
        
        dataptrp[offset+2][0] = (DCTELEM)
            RIGHT_SHIFT(z1 + MULTIPLY(tmp12, FIX_0_765366865), /* c2-c6 */
            CONST_BITS-PASS1_BITS);
        dataptrp[offset+6][0] = (DCTELEM)
        ...
        dataptrp[offset+1][0] = (DCTELEM) RIGHT_SHIFT(tmp0, CONST_BITS-PASS1_BITS);
        dataptrp[offset+3][0] = (DCTELEM) RIGHT_SHIFT(tmp1, CONST_BITS-PASS1_BITS);
        dataptrp[offset+5][0] = (DCTELEM) RIGHT_SHIFT(tmp2, CONST_BITS-PASS1_BITS);
        dataptrp[offset+7][0] = (DCTELEM) RIGHT_SHIFT(tmp3, CONST_BITS-PASS1_BITS);
    ...
    #pragma omp parallel for private(tmp0,tmp1,tmp2,tmp3,tmp10,tmp11,tmp12,tmp13,z1)
    for(ctr = 0 ; ctr < DCTSIZE ; ctr++){
        tmp0 = dataptrp[ctr+DCTSIZE*0][0] + dataptrp[ctr+DCTSIZE*7][0];
        tmp1 = dataptrp[ctr+DCTSIZE*1][0] + dataptrp[ctr+DCTSIZE*6][0];
        tmp2 = dataptrp[ctr+DCTSIZE*2][0] + dataptrp[ctr+DCTSIZE*5][0];
        tmp3 = dataptrp[ctr+DCTSIZE*3][0] + dataptrp[ctr+DCTSIZE*4][0];
        ...
        dataptrp[ctr+DCTSIZE*0][0] = (DCTELEM) RIGHT_SHIFT(tmp10 + tmp11, PASS1_BITS);
        dataptrp[ctr+DCTSIZE*4][0] = (DCTELEM) RIGHT_SHIFT(tmp10 - tmp11, PASS1_BITS);

        z1 = MULTIPLY(tmp12 + tmp13, FIX_0_541196100);       /* c6 */
        /* Add fudge factor here for final descale. */
        z1 += ONE << (CONST_BITS+PASS1_BITS-1);

        dataptrp[ctr+DCTSIZE*2][0] = (DCTELEM)
        RIGHT_SHIFT(z1 + MULTIPLY(tmp12, FIX_0_765366865), /* c2-c6 */
            CONST_BITS+PASS1_BITS);
        dataptrp[ctr+DCTSIZE*6][0] = (DCTELEM)
        RIGHT_SHIFT(z1 - MULTIPLY(tmp13, FIX_1_847759065), /* c2+c6 */
            CONST_BITS+PASS1_BITS);
        ...
        dataptrp[ctr+DCTSIZE*1][0] = (DCTELEM) RIGHT_SHIFT(tmp0, CONST_BITS+PASS1_BITS);
        dataptrp[ctr+DCTSIZE*3][0] = (DCTELEM) RIGHT_SHIFT(tmp1, CONST_BITS+PASS1_BITS);
        dataptrp[ctr+DCTSIZE*5][0] = (DCTELEM) RIGHT_SHIFT(tmp2, CONST_BITS+PASS1_BITS);
        dataptrp[ctr+DCTSIZE*7][0] = (DCTELEM) RIGHT_SHIFT(tmp3, CONST_BITS+PASS1_BITS);
...
int main(void){
    ...
    // double time_data[TOTAL_FUNC_CALL];
    ...
    for(int i = 0 ; i<TOTAL_FUNC_CALL ; i++){
        ...
        // time_data[i] = ((double) end.tv_sec - (double) start.tv_sec) + ((double) end.tv_usec - (double) start.tv_usec)/1000000.0;   // unit is us
